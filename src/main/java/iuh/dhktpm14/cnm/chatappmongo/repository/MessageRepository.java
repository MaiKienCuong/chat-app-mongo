...
//    Message getLastMessageOfRoom(String roomId);
    ...
    @Query(value = "{roomId: ?0, userDelete: {$nin: ?1}, content: {$regex: /<a href=\".*\">.*<\\/a>/ }}", sort = "{createAt: -1}")
    Page<Message> findAllByTypeLink(String roomId, List<String> userDelete, Pageable pageable);
